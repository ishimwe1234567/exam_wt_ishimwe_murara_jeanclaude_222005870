...
<body style="background-color: lightblue;">
  ...
  <body style="background-color: yellowgreen;">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add'])) {
        ...
        $stmt = $connection->prepare("INSERT INTO moduleresources (resource_id, module_id, resource_type, resource_title, resource_description, resource_url) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("iissss", $resource_id, $module_id, $resource_type, $resource_title, $resource_description, $resource_url);

        if ($stmt->execute()) {
            echo "New record has been added successfully.<br><br><a href='moduleresources.php'>Back to Form</a>";
        } else {
    ...
    <section>
      ...
      <table>
        ...
        <?php
        $sql = "SELECT * FROM moduleresources";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td><a style='padding:4px' href='delete_moduleresource.php?resource_id={$row['resource_id']}'>Delete</a></td> 
                        <td><a style='padding:4px' href='update_moduleresource.php?resource_id={$row['resource_id']}'>Update</a></td> 
                      </tr>";
            }
        } else {
